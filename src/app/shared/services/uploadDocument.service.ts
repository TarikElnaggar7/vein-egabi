...
import { BehaviorSubject, EMPTY, from, ReplaySubject, Subject, forkJoin, of, Observable } from "rxjs";
import { map, groupBy, mergeMap, toArray, take } from "rxjs/operators";
import { UtilityManager } from "../utilities/UtilityManager";
import { ComponentsIDs, DbTables, DescrColums, FormsIDs, KeyValueColums, LookupFlag, URLs } from "../utilities/Constants";
...
export class UploadDocumentService implements OnInit, OnDestroy {
  private result: any;
  ...
  calcObjLookup() {
    const calcObjIds = <any[]>[
      ...
      {
        name: "PAGE_ID ",
        valueKey: !this.pageId ? FormsIDs.UploadDocument : this.pageId,
      },
    ];
  ...
  //
  prepareUploadPdfReq(event, objectdata: UploadDocumentModel, documentType: string, moduleNumber: number,
    deptRecordCode: any, docTypeId: any) {
    this.toastr.info(`Uploading your file..`, "", {
      positionClass: 'toast-top-center', progressBar: true,
      timeOut: 10000,
    });
    // Read File
      ...
      if (!file.type.match('application/pdf.*')) {
        ...
        fileReader.onload = (fileLoadedEvent) => {
          if (fileLoadedEvent.currentTarget.readyState === 1) {
            ...
            // this.uploadPdf(jsonDataRequest);
            this.utilService.postService(URLs.postData, jsonDataRequest)
              .subscribe(
                (res) => {
                  if (res.status == 1) {
    ...
    // return forkJoin([this.response1]);
  }
    ...
    this.utilService
      ...
      .subscribe(
        (res) => {
          ...
          // binaryData.push(this.apiUrl + "/" + res["body"]["objectdata"][0]);
          // const downloadURL = URL.createObjectURL(new Blob(binaryData, { type: "application/pdf" }));

          // window.open(binaryData[0]);
          const downloadLink = document.createElement("a");
          downloadLink.href = this.apiUrl + "/" + res["body"]["objectdata"][0];
  ...
  async deletePdf(document: number | string): Promise<Observable<any>> {
    const jsonDataRequest = {
      metadata: {
        pageId: FormsIDs.UploadDocument,
        componentId: ComponentsIDs.Default,
        // oprNo: this.oppNumber,
        //  oprNo: await this.operationNumber,
        funcMode: "D",
        usrNo: SessionManager.getUserNo(),
        roleId: SessionManager.getRoleId(),
        langCode: SessionManager.getLanguage(),
    ...
    this.spinner.show();
    return this.utilService.deleteService(URLs.deleteData, jsonDataRequest);
    // .subscribe(
    //   (res) => {
    //     // Getting required tables calcs and tables data after uploading a file
    //     // this.loadDocumentColumns();
